...
function clean_file()
{
    $.getJSON('index.php?app=widget_store&act=clean_file', function(data){
        if (!data.done)
        {
            alert(data.msg);
        ...
        else
        {
            if (confirm(data.msg))
            {
                $.getJSON('index.php?app=widget_store&act=clean_file&continue', function(rzt){
                    alert(rzt.msg);
                });
            }
        }
...
<div id="rightTop">
    <p>挂件管理</p>
    <ul class="subnav">
        <li><a class="btn1" href="index.php?app=widget">商城挂件</a></li>
        <li><span>店铺挂件</span></li>
        <li>[<a href="javascript:void(0);" onclick="clean_file();" title="孤立文件是指被上传到服务器上，但实际并没有被引用的文件，重复配置有上传文件的挂件，会产生孤立文件，因此需要定时清理这些文件以释放硬盘空间">清理店铺孤立文件</a>]</li>
    </ul>
</div>
...
?>
        <tr class="tatr2">
            ...
            <td class="handler">
                <a href="index.php?app=widget_store&amp;act=edit&name=<?php echo $this->_var['widget']['name']; ?>&file=script">编辑脚本</a>
                |
                <a href="index.php?app=widget_store&amp;act=edit&name=<?php echo $this->_var['widget']['name']; ?>&file=template">编辑模板</a>
                </td>
        </tr>
        <?php endforeach; else: ?>
        <tr class="no_data">
