...
<div class="tdare">
    <table width="100%" cellspacing="0" class="dataTable">
        <?php if ($this->_var['jiangpin_list']): ?>
        <tr class="tatr1">
        ...
        <?php endif; ?>
        <?php $_from = $this->_var['jiangpin_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'val');if (count($_from)):
...
?>
        <tr class="tatr2">
            ...
            <td><?php echo htmlspecialchars($this->_var['val']['title']); ?></td>
            <td class="table_center"><?php echo htmlspecialchars($this->_var['val']['num']); ?> </td>
             <td class="table_center"><?php echo htmlspecialchars($this->_var['val']['gailv']); ?> </td>
             <td class="table_center"><?php echo $this->_var['val']['money']; ?> </td>
             <td class="table_center"><?php echo local_date("Y-m-d",$this->_var['val']['time']); ?> </td>
            <td class="handler">
            <a href="index.php?module=dazhuanpan&amp;act=edit&amp;id=<?php echo $this->_var['val']['id']; ?>">编辑</a>
                |
                <a href="javascript:if(confirm('您确定要删除它吗？'))window.location = 'index.php?module=dazhuanpan&amp;act=drop&amp;id=<?php echo $this->_var['val']['id']; ?>';">删除</a>  </td>
        </tr>
        <?php endforeach; else: ?>
        <tr class="no_data">
        ...
        <tfoot>
            ...
            <?php if ($this->_var['jiangpin_list']): ?>
                ...
                <td colspan="3" id="batchAction">
                    ...
                    <input class="formbtn batchButton" type="button" value="删除" name="id" uri="index.php?module=dazhuanpan&act=drop" presubmit="confirm('您确定要删除它吗？');" />
                </td>
            <?php endif; ?>
            </tr>
        </tfoot>
