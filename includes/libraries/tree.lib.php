...
class Tree extends Object
{
     ...
     */
    function setTree($nodes, $id_field, $parent_field, $value_field,$value_pic='')
    {
        $this->value_field = $value_field;		
	$this->value_pic = $value_pic;
    ...
    function getcatpic($id)
    {
        return $this->data[$id]['catpic'];
    }
    ...
    function getArrayList($root = 0 , $layer = NULL)
    {
        ...
        foreach ($this->child[$root] as $id)
        {
            ...
            $data[] = array('id' => $id, 'value' => $this->getValue($id),'catpic' =>$this->getcatpic($id), 'children' => $this->child[$id] ? $this->getArrayList($id , $layer) : array());
        }
        return $data;
    }
