...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
		...
		tpl.find('*[item="gh_price"]').append('<input type="hidden" name="gh_price['+ item.spec_id +']" value="' + item.gh_price + '" />' + item.gh_price);
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
		...
		tpl.find('*[item="gh_price"]').val(item.gh_price);
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
			...
			var gh_price = $.trim($(this).find('*[item="gh_price"]').val());
            ...
            if(SPEC.spec_qty == 1){ // 一个规格
                ...
                eval('if(spec_' + spec_pos + ' || (!spec_' + spec_pos + ' && !price && !gh_price && !stock && !sku)){}else{complate = false;}');
            }else{ // 两个规格
                if((spec_1 && spec_2) || (!spec_1 && !spec_2 && !price && !gh_price && !stock && !sku)){
            ...
            item != ';' && SPEC.specs.push({
				...
				'gh_price':number_format(gh_price, 2),
