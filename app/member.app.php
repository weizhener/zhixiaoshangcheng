...
class MemberApp extends MemberbaseApp {
    ...
    function send_code() {
		...
		if (!$mobile) {
         ...
         if(!ecm_json_encode($ms->user->check_email($mobile)))
		 ...
		 $code=$mobile.$make_code;

	

		  $this->_mailto($mobile, addslashes('邮箱验证'), "你的验证码是".$make_code);
    ...
    function cmc() {
        ...
        $confirm_code = empty($_GET['confirm_code']) ? '' : trim($_GET['confirm_code']);
		 ...
		 $email = empty($_GET['email']) ? '' : trim($_GET['email']);
		  ...
		  $confirm_code =md5($email.$confirm_code);

        if (empty($_SESSION['code']) || !$confirm_code) {
