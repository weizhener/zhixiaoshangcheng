...
class My_wxmenuApp extends StoreadminbaseApp {
    ...
    public function create_weixin_menu() {
        ...
        if ($ACCESS_LIST['access_token'] != '') {
            ...
            $msg = Init_Weixin::curl_menu($access_token, preg_replace("#\\\u([0-9a-f]+)#ie", "iconv('UCS-2BE', 'UTF-8', pack('H4', '\\1'))", $data));
            if ($msg['errmsg'] == 'ok') {
	...
	function ajax_col()
   {
       ...
       $column = empty($_GET['field']) ? '' : trim($_GET['field']);
       $value  = isset($_GET['val']) ? trim($_GET['val']) : '';
	$this->my_wxmenu_mod->edit($id,array($column=>$value));   
