...
class SearchApp extends MallbaseApp {
    ...
    function index() {
		...
		foreach($goods_list as $k=>$v){
			$goods_list[$k]['sales_info'] = $v['sales'] + $v['virtual_seles'];
		}
...
function get_all_category_tree($cate_id) {
    ...
    function store() {
        ...
        foreach ($stores as $key => $store) {
            $goods_list = $model_goods->find(array(
                'conditions' => 'is_pass=1 AND '.'store_id=' . $store['store_id'],
                'order' => 'add_time desc',
                'limit' => 10,
    ...
    function _get_goods_conditions($param) {
        /* 组成查询条件 */
        $conditions = " g.if_show = 1 AND g.closed = 0 AND g.is_pass = 1 AND s.state = 1"; // 上架且没有被禁售，店铺是开启状态,
        if (isset($param['keyword'])) {
