...
class My_goodsApp extends StoreadminbaseApp {
 ...
 $goods= $this->_goods_mod->get($id);
   ...
   $this->assign('goods', $goods);
	 ...
	 $this->display('my_goods.out.html');
	}
    function index() {
        /* 取得店铺商品分类 */
    ...
    function _get_goods($conditions, &$page) {
        ...
        if (isset($_GET['sort']) && isset($_GET['order'])) {
            ...
            if (!in_array($sort, array('goods_name', 'cate_id', 'brand', 'price', 'gh_price','stock', 'if_show', 'recommended', 'closed'))) {
                $sort = 'goods_id';
                $order = 'desc';
            }
 ...
 /* 赋值运费模板 - psmb */
        ...
        if (!IS_POST) {
            ...
            foreach ($data as $key => $val) {
                if (empty($default_spec)) {
                    $default_spec = array('price' => $val['price']);                                        $default_spec = array('gh_price' => $val['gh_price']);
                }
    ...
    function save_spec($spec) {
		...
		foreach ($spec['gh_price'] as $key => $val)
        {
            $data[$key]['gh_price'] = $this->_filter_price($val);
        }
        ...
        if (!IS_POST) {
            ...
            foreach ($records as $record) {
                ...
                if ($record['sale_attr']) { // 有规格
                    ...
                    if ($spec_qty > 2 || !$spec_info) { // 有两个以上规格或淘宝接口没有获取到属性，视无规格处理
                        ...
                        $spec_data[0] = array(
							...
							'gh_price'  => $this->_filter_price($_POST['gh_price'][$key]),
                    ...
                    $spec_data[0] = array(
					   'gh_price' => $this->_filter_price($_POST['gh_price']),
                        'goods_id' => $goods_id,
                        'price' => $this->_filter_price($record['price']),
                        'stock' => intval($record['stock']),
                    );
                ...
                foreach ($spec_data as $spec) {
                    ...
                    if (empty($default_spec)) {
                        $default_spec = array('default_spec' => $spec_id, 'gh_price' => $spec['gh_price'],'price' => $spec['price']);
                    }
                }
...
*/
        ...
        if (empty($delivery)) {
            $this->show_warning('请至少添加一个运费模板才能发布商品', '添加运费模板', 'index.php?app=my_delivery');
            return false;
        }
    ...
    function _get_post_data($id = 0) {
        ...
        switch ($goods['spec_qty']) {
			...
			//		'gh_price' => $this->_filter_price($_POST['gh_price']),
                ...
                );                                if($id==0)                {                    $specs[intval($_POST['spec_id'])]['gh_price']=$this->_filter_price($_POST['gh_price']);                }
                break;
            case 1: // 一个规格
                ...
                foreach ($spec_data as $key => $spec_1) {
                    ...
                    if ($spec_1) {
						...
						//		'gh_price'  => $this->_filter_price($_POST['gh_price'][$key]),
                            ...
                            );                                                        if($id==0)                            {                                $specs[$key]['gh_price']=$this->_filter_price($_POST['gh_price'][$key]);                            }
                        } else {  // 新增的规格
                            $specs[$key] = array(
                                'spec_1' => $spec_1,
                                'price' => $this->_filter_price($_POST['price'][$key]),
